"""``chipsage-build`` — build the Tier-1 SQLite index from CMSIS-SVD files.

Example::

    chipsage-build --svd data/svd/RP2040.svd data/svd/RP2350.svd -o chipsage.db

Reads only local files (no network), rebuilds the output database from scratch, and prints a
one-line summary per chip. ``--strict`` makes any ingest violation a non-zero exit so CI can
fail on a poisoned SVD.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    missing = [str(p) for p in args.svd if not p.is_file()]
    if missing:
        print(f"error: SVD file(s) not found: {', '.join(missing)}", file=sys.stderr)
        return 2
